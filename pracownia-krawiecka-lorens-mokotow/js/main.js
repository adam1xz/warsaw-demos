document.addEventListener('DOMContentLoaded', function () {
  ...
  if (navbar) {
    var onScroll = function () {
      ...
      } else if (!navbar.classList.contains('scrolled') || window.location.pathname.indexOf('index.html') !== -1 || window.location.pathname === '/' ) {
        if (window.scrollY <= 40 && navbar.dataset.forceScrolled !== 'true') {
  ...
  // Fade-in on scroll
  var revealTargets = document.querySelectorAll('.service-card, .swatch, .about-grid, .testimonial blockquote');
  ...
  if (bookingForm) {
    bookingForm.addEventListener('submit', function (e) {
      ...
      if (!time.value) {
        ...
        valid = false;
      } else if (time.value < '09:00' || time.value > '18:00') {
        timeError.textContent = 'Wybierz godzinę w zakresie 9:00–18:00.';
        valid = false;
      }
  ...
  if (masonry && lightbox) {
    ...
    masonry.querySelectorAll('.swatch').forEach(function (item) {
