...
if (bookingForm) {
  bookingForm.addEventListener('submit', (e) => {
    ...
    const pet = bookingForm.querySelector('#b-pet');
    ...
    if (!name.value.trim()) error = 'Podaj proszę imię i nazwisko właściciela.';
    else if (!phonePattern.test(phone.value.trim())) error = 'Podaj prawidłowy numer telefonu.';
    else if (!service.value) error = 'Wybierz rodzaj wizyty.';
    else if (!pet.value) error = 'Wybierz gatunek zwierzęcia.';
    ...
    msg.textContent = `Dziękujemy, ${name.value.trim()}! Prośba o wizytę (${service.options[service.selectedIndex].text}, ${pet.options[pet.selectedIndex].text}, ${date.value} o ${time.value}) została wysłana. Zadzwonimy, aby potwierdzić termin.`;
...
if (lightbox) {
  document.querySelectorAll('.gallery-card').forEach(card => {
    card.addEventListener('click', () => {
      ...
      lightbox.querySelector('.lightbox-card').style.background = bg;
