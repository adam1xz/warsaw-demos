document.addEventListener('DOMContentLoaded', function () {
  ...
  if (bookingForm) {
    bookingForm.addEventListener('submit', function (e) {
      ...
      if (!name.value.trim() || !phonePattern.test(phone.value.trim()) || !date.value || !time.value || !guests.value) {
        msg.textContent = 'Uzupełnij poprawnie wszystkie pola (telefon: same cyfry, min. 7 znaków).';
        msg.className = 'form-msg show error';
        return;
      }

      msg.textContent = 'Dziękujemy, ' + name.value.trim() + '! Rezerwacja na ' + guests.value + ' os. w dniu ' + date.value + ' o ' + time.value + ' została przyjęta. Zadzwonimy, aby potwierdzić.';
